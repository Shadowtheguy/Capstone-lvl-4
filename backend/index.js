...
app.post("/mtgdecks", async (req, res) => {
  const { deck_title, deck_author, format, card_list, custom_cards } = req.body;
...
app.put("/mtgdecks/:id", async (req, res) => {
  const { deck_title, deck_author, format, card_list, custom_cards } = req.body;
...
app.delete("/mtgdecks/:id", async (req, res) => {
  const { data, error } = await supabase
    .from("MTG Deck Manager")
    .delete()
    .eq("id", req.params.id);
